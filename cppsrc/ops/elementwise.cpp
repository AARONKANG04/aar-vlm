...
namespace vlm {
    ...
    Tensor sub(const Tensor& a, const Tensor& b) {
        ...
        if (!a.requires_grad && !b.requires_grad) {
            return sub_no_grad(a, b);
        }
        auto fn = std::make_shared<SubFunction>();
        fn->record_input(a);
        fn->record_input(b);
        Tensor out = sub_no_grad(a, b);
        out.requires_grad = true;
        out.grad_fn = fn;
        return out;
    }
    ...
    Tensor gelu(const Tensor& a) {
        ...
        if (!a.requires_grad) {
            return a.device == Device::CPU ? gelu_cpu(a) : gelu_cuda(a);
        }
        auto fn = std::make_shared<GeluFunction>();
        fn->record_input(a);
        fn->saved_x = a;
        Tensor out = a.device == Device::CPU ? gelu_cpu(a) : gelu_cuda(a);
        out.requires_grad = true;
        out.grad_fn = fn;
        return out;
    }
    ...
    Tensor add_bias(const Tensor& x, const Tensor& bias) {
        ...
        if (!x.requires_grad && !bias.requires_grad) {
            return add_bias_no_grad(x, bias);
        }
        auto fn = std::make_shared<AddBiasFunction>();
        fn->record_input(x);
        fn->record_input(bias);
        fn->bias_size = bias.shape[0];
        Tensor out = add_bias_no_grad(x, bias);
        out.requires_grad = true;
        out.grad_fn = fn;
        return out;
    }
